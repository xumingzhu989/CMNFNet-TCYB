...
from model import basicnetVgg256NestFuse
...
# Display the saliency map and ground truth during training
...
def train(loss_fn, args):
    ...
    # Training data conversion and loading
    value_scale = 255
    ...
    # #################################################20220829VGG#######################################################
    net = basicnetVgg256NestFuse.CMNFNet(args.block_num, args.block_nod, args.img_dim, args.cov_loop,
    ...
    date_str = str(datetime.datetime.now().date())  # Get the current date
    # Train the model
    model.train()
    lossAvg = 0
    for epoch in range(args.epoch_num):
        for i, (input, gt, _) in enumerate(train_loader):
            ...
            loss.backward()
            optimizer.step()
            if (i + 1 == len(train_loader)):  # Display the visual results every epoch
        ...
        print("第%d-th epoch learning rate：%f" % (epoch+1, optimizer.param_groups[0]['lr']))
        scheduler.step()
        if ((epoch + 1) % 1 == 0) or (epoch + 1 == args.epoch_num):
            model_folder = args.model_path + date_str + '/'
            check_makedirs(model_folder)
            torch.save(model.state_dict(), model_folder + 'CMNFNet-vgg-' + str(epoch + 1) + '.pth')
...
def test(args):
    ...
    test_transform = transform.Compose([
        ...
        transform.ToTensor(),
        transform.Normalize(mean=mean, std=std)])
    to_pil_img = transforms.ToPILImage()  # Convert the output tensor to a PIL image

    date_str = str(datetime.datetime.now().date())  # Get the current date
    results_folder = args.results_folder + date_str  # Name the folder with the current date
    ...
    for now in epoch_list:
        model = net.to(device)
        model_dir = args.model_path + '2025-11-21/' + 'CMNFNet-vgg-' + str(now) + ".pth"  # the path/file to save the trained model params.
        results_folder_now = results_folder + "/" + 'CMNFNet-vgg-' + str(now)
...
if __name__ == '__main__':
    args = get_parser()
    Flag_train_test = 0 # 0 is train; and 1 is test.
