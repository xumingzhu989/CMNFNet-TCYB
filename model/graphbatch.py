...
# 将CNN的批处理数据转换为PYG中的批处理数据格式，4*4图像块的话，以每一个图像块作为基本处理单元，每个图像块内的节点互联
def graph_batch_trans_block_singlepatch(adj, bndnum, thr, B, x_t, node_num, bknum):  # x is [B, C, node_num]
    edge_index = []
    edge_attr = []
    data = []
    batch = []
    # adj = torch.where(adj < 1.0, torch.zeros_like(adj), adj)  # 将adj中相似度值较小的置为0
    # adj = torch.where(adj < thr, torch.zeros_like(adj), adj)  # 将adj中相似度值较小的置为0
    adj_new = torch.zeros_like(adj)
    ## 把patch块内的图节点间的相似性关系放在新矩阵的对角线上
    ...
    graph_batch = torch.cat(batch, dim=0).to(x_t.device)#.cuda()
...
# 将CNN的批处理数据转换为PYG中的批处理数据格式，4*4图像块的话，以每四个图像块作为基本处理单元，每四个图像块内的节点互联
def graph_batch_trans_block4to2(adj, bndnum, thr, B, x_t, node_num, bknum):  # x is [B, C, node_num]
    edge_index = []
    edge_attr = []
    data = []
    batch = []
    adj = torch.where(adj < 0.6, torch.zeros_like(adj), adj)  # 将adj中相似度值较小的置为0
    adj_new = torch.zeros_like(adj)
    # 将4*4图像块合并为2*2
    ...
    graph_edge_index = torch.cat(edge_index, dim=1)
    graph_edge_attr = torch.cat(edge_attr, dim=0)
    graph_batch = torch.cat(batch, dim=0).cuda()
    return graph_data, graph_edge_index, graph_edge_attr, B, graph_batch  # [node_num_batch, C], [2, node_connect_batch], [node_connect_batch]
...
def thr_select(adj, k):  # adj是B*N*N的邻接矩阵，k是取邻居节点个数
    ...
    val, _ = adj.topk(k, dim=2, largest=True, sorted=True)  # val is B*N*k
    thr = val[:,:,k-1]  # 取第k个值作为阈值，维度为B*N
    return thr.unsqueeze(2)  # 维度为B*N*1


# 将CNN的批处理数据转换为PYG中的批处理数据格式，4*4图像块的话，以每16个图像块作为基本处理单元，每16个图像块内的节点互联
def graph_batch_trans_block4to1(adj, bndnum, thr, B, x_t, node_num, bknum):  # x is [B, C, node_num]
    edge_index = []
    edge_attr = []
    data = []
    batch = []
    # thr = thr_select(adj, k=k)
    # adj = torch.where(adj < thr, torch.zeros_like(adj), adj)  # 将adj中相似度值较小的置为0
    # adj = torch.where(adj < 0.0, torch.zeros_like(adj), adj)  # 将adj中相似度值较小的置为0
    # img_show(adj)
    for i in range(B):
...
# # 将CNN的批处理数据转换为PYG中的批处理数据格式，以16个图像块作为处理单元
# def graph_batch_trans_block4to1xxx(adj, bndnum, thr, B, x_t, node_num, bknum):  # x is [B, C, node_num]
#     edge_index = []
#     edge_attr = []
#     data = []
#     batch = []
#     thr = thr_select(adj, k=9)
#     adj = torch.where(adj < thr, torch.zeros_like(adj), adj)  # 将adj中相似度值较小的置为0
#     # img_show(adj)
#     for i in range(B):
#         edge_index_i, edge_attr_i = dense_to_sparse(adj[i])  # [2, node_connect], [node_connect]
