...
class ChannelAttention(nn.Module):
    def __init__(self, ch, ratio=16):
        ...
        self.p = nn.Parameter(torch.ones(ch, 1, 1)).cuda()
    ...
    def forward(self, xin):
        ...
        xin = self.p * xin + (1 - self.p) * (max - xin)
...
class GraphProcess(nn.Module):
    ...
    def forward(self, x, Q, g):  #
        ...
        rg = self.reproj(Q, g)
        return rg  # g_cgi is B C node_num
...
class CalculateUnitAE(nn.Module):
    def __init__(self, in_Channel1=0, in_Channel2=0, in_Channel3=0, out_Channel=0, SA_Channel=0):
        super(CalculateUnitAE, self).__init__()
        self.sig = nn.Sigmoid()
        self.AE1 = AE(in_Channel1)
        self.AE2 = AE(in_Channel2)
        self.AE3 = AE(in_Channel3)
        if (SA_Channel != 0):
            self.convSA = BasicConv2d(SA_Channel, 1, 3, padding=1)
        self.conv = nn.Sequential(BasicConv2d(in_Channel1 + in_Channel2 + in_Channel3, out_Channel, 3, padding=1), BasicConv2d(out_Channel, out_Channel, 3, padding=1))  # 这里是每个小模块用到的卷积，可以改成其他的
...
class decoder(nn.Module):
    def __init__(self):
        ...
        self.c55 = CalculateUnitAE(a5_channel, a5_channel, a4_channel, a4_channel, 0)
        self.c54 = CalculateUnitAE(a4_channel, a4_channel, a3_channel, a3_channel, a4_channel)
        self.c53 = CalculateUnitAE(a3_channel, a3_channel, a2_channel, a2_channel, a3_channel)
        self.c52 = CalculateUnitAE(a2_channel, a2_channel, a1_channel, a1_channel, a2_channel)
        self.c51 = CalculateUnitAE(a1_channel, a1_channel, a1_channel, a1_channel, a1_channel)

        self.c45 = CalculateUnitAE(a4_channel, a4_channel, a3_channel, a3_channel, a4_channel)
        self.c44 = CalculateUnitAE(a3_channel, a2_channel, 0, a2_channel, a3_channel)
        self.c43 = CalculateUnitAE(a2_channel, a1_channel, 0, a1_channel, a2_channel)
        self.c42 = CalculateUnitAE(a1_channel, a1_channel, 0, a1_channel, a1_channel)

        self.c35 = CalculateUnitAE(a3_channel, a3_channel, a2_channel, a2_channel, a3_channel)
        self.c34 = CalculateUnitAE(a2_channel, a1_channel, 0, a1_channel, a2_channel)
        self.c33 = CalculateUnitAE(a1_channel, a1_channel, 0, a1_channel, a1_channel)

        self.c25 = CalculateUnitAE(a2_channel, a2_channel, a1_channel, a1_channel, a2_channel)
        self.c24 = CalculateUnitAE(a1_channel, a1_channel, 0, a1_channel, a1_channel)

        self.c15 = CalculateUnitAE(a1_channel, a1_channel, 0, a1_channel, a1_channel)
    ...
    def forward(self, xC5, xC4, xC3, xC2, xC1, xG5, xG4, xG3, xG2, xG1):
        x55 = self.c55(xC5, xG5, xG4, 0)
        x45 = self.c45(x55, xG4, self.dw2(xG3), x55)
        ...
        return self.up2(s1), self.up4(s2), self.up8(s3), self.up16(s4), self.up16(s5), self.up2(r2), self.up2(r3), self.up2(r4), self.up2(r5)
...
class CMNFNet(nn.Module):
    def __init__(self,bnum, bnod, dim,loop,bias,init, k, res_pretrained):
        ...
        self.sig = nn.Sigmoid()
        ################################################################################################################################
        c1, c2, c3, c4, c5 = 64, 256, 512, 1024, 2048
        # Resnet
        self.conv2 = nn.Sequential(BasicConv2d(c2, 128, kernel_size=3, padding=1),
                                   BasicConv2d(128, 128, kernel_size=1, padding=0))  # 256-128

        self.conv3 = nn.Sequential(BasicConv2d(c3, c2, kernel_size=3, padding=1),
                                   BasicConv2d(c2, c2, kernel_size=1, padding=0))  # 512-256

        self.conv4 = nn.Sequential(BasicConv2d(c4, c2, kernel_size=3, padding=1),
                                   BasicConv2d(c2, c2, kernel_size=1, padding=0))  # 1024-256

        self.conv5 = nn.Sequential(BasicConv2d(c5, c2, kernel_size=3, padding=1),
                                   BasicConv2d(c2, c2, kernel_size=1, padding=0))  # 2048-256
        resnet = res.resnet50_XMZ(pretrained=res_pretrained)
        self.layer1 = nn.Sequential(resnet.conv1, resnet.bn1, resnet.relu)
        self.layer2, self.layer3, self.layer4, self.layer5 = nn.Sequential(resnet.maxpool,
        ...
        self.pos_embed3 = nn.Parameter(torch.zeros(1, 256, 32, 32))
        nn.init.trunc_normal_(self.pos_embed3, std=.02)
        self.pos_embed4 = nn.Parameter(torch.zeros(1, 256, 16, 16))
        ...
        # GCN
        self.convg1 = nn.Sequential(BasicConv2d(3, 2 * dim, kernel_size=3, stride=2, padding=1), BasicConv2d(2*dim, 2*dim, kernel_size=1, stride=1, padding=0))  # channel:3-64, size:128

        self.convg2 = nn.Sequential(BasicConv2d(2*dim, 4*dim, kernel_size=3, stride=2, padding=1), BasicConv2d(4*dim, 4*dim, kernel_size=1, stride=1, padding=0)) # channel: 64-128, size: 64

        self.convg3 = nn.Sequential(BasicConv2d(4 * dim, 8 * dim, kernel_size=3, stride=2, padding=1), BasicConv2d(8*dim, 8*dim, kernel_size=1, stride=1, padding=0))  # channel: 128-256, size: 32
        self.GCNblock3 = graph_block_d(int(bnum / 1), int(bnod / 1), 8 * dim, int(loop / 1), bias, k)

        self.convg4 = nn.Sequential(BasicConv2d(8 * dim, 8 * dim, kernel_size=3, stride=2, padding=1), BasicConv2d(8*dim, 8*dim, kernel_size=1, stride=1, padding=0))  # channel: 256-256, size: 16
        self.GCNblock4 = graph_block_d(int(bnum / 1), int(bnod / 1), 8 * dim, int(loop / 1), bias, k)

        self.convg5 = nn.Sequential(BasicConv2d(8 * dim, 8 * dim, kernel_size=3, stride=1, padding=1), BasicConv2d(8*dim, 8*dim, kernel_size=1, stride=1, padding=0))  # channel: 256-256, size: 16
        self.GCNblock5 = graph_block_d(int(bnum / 1), int(bnod / 1), 8 * dim, int(loop / 1), bias, k)
        ###############################################
    ...
    def forward(self, Xin):
        ...
        xG1 = self.convg1(Xin)
        #########################################
        xc2 = self.layer2(xC1)  # 1/4, 256
        xC2 = self.conv2(xc2)  # 1/4, 128
        xG2 = self.convg2(xG1)
        #########################################
        xc3 = self.layer3(xc2)  # 1/8, 512
        xC3 = self.conv3(xc3)  # 1/8, 256
        xg3 = self.convg3(xG2)
        xG3 = self.GCNblock3(xg3) + self.pos_embed3
        #########################################
        xc4 = self.layer4(xc3)  # 1/16, 1024
        xC4 = self.conv4(xc4)  # 1/16, 256
        xg4 = self.convg4(xG3)
        xG4 = self.GCNblock4(xg4) + self.pos_embed4
        #########################################
        xc5 = self.layer5(xc4)  # 1/16, 2048
